 ...
 * ---------------------------------------------------------------------------- */
unsigned char IicConfig(unsigned int AudioDeviceID)
{
	XIicPs_Config *ConfigAudio;
	int Status;

	/* Initialise the IIC driver so that it's ready to use */

	// Look up the configuration in the config table
	ConfigAudio = XIicPs_LookupConfig(AudioDeviceID);
	if(NULL == ConfigAudio) {
		return XST_FAILURE;
	}

	// Initialise the IIC driver configuration
	Status = XIicPs_CfgInitialize(&IicAudio, ConfigAudio, ConfigAudio->BaseAddress);
	 ...
	 */
	Status = XIicPs_SelfTest(&IicAudio);
	if (Status != XST_SUCCESS) {
		xil_printf("IIC FAILED \r\n");
		return XST_FAILURE;

	}
	xil_printf("IIC Codec Passed\r\n");

	//Set the IIC serial clock rate.
	Status = XIicPs_SetSClk(&IicAudio, IIC_SCLK_RATE);
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
...
void AudioWriteToReg(u8 u8RegAddr, u16 u16Data) {
	...
	XIicPs_MasterSendPolled(&IicAudio, u8TxData, 2, IIC_SLAVE_ADDRAudio);
	while(XIicPs_BusIsBusy(&IicAudio));
}
